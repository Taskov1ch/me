...
$(document).ready(function () {
	...
	// Новая функция для проверки ширины экрана и изменения слайдов
	const checkDeviceAndModifySlides = () => {
		if ($(window).width() < 768) {
			$slides.not("#links").hide();
			$("#links").show().addClass("active").css("opacity", "1");
			currentSlide = $slides.index($("#links"));
			$indicatorsUl.hide();
			history.replaceState(null, "", `?slide=links`);
			document.title = "Мои ссылки | Taskov1ch";
		} else {
			$slides.show();
			$indicatorsUl.show();
			updateSlider();
	...
	checkDeviceAndModifySlides();
	$(window).on("resize", checkDeviceAndModifySlides);
